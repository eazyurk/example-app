...
@section('content')
    <div class="w-4/5 m-auto">
        <div class="mb-4 flex">
            <div>
                <h1 class="text-xl font-bold decoration-gray-400">
                    Batch {{ $batch->batch_code }} updaten
                </h1>
            </div>
            <div class="flex-1">
        ...
        <form action="{{ route('batches.update.store', $batch->id) }}" class="border-black bg-gray-50 p-3 rounded pb-12" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-6">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="batch_code" name="batch_code" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $batch->batch_code }}" required="">
            </div>
            <div class="mb-6">
                <label for="amount" class="block mb-2 text-sm font-medium text-gray-900">Aantal</label>
                <input type="number" id="amount" name="amount" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $batch->amount }}" required="">
            </div>
