    ...
    <div class="w-4/5 m-auto">
        ...
        <form action="{{ route('article.update.store', $article->id) }}" class="border-black bg-gray-50 p-3 rounded pb-12" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="article_code" name="article_code" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ $article->article_code }}" required="">
            </div>
            <button type="submit" class="float-right text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Opslaan</button>
        </form>
    </div>
@stop
